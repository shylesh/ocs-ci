""" A module for all rook functionalities and abstractions.
 ...
 This module has rook related classes, support for functionalities
 to work with rook cluster. This works with assumptions that an OCP cluster
 is already functional and proper configurations are made for interaction.
...
class Rook(object):
    ...
    but in future rookenv should be handled here.
...
class RookCluster(object):
    ...
    Methods:
       add_mon(**kw) : add a new mon
       remove_mon(id): Remove a mon with id id
       add_osd(**kw):  add a new osd to cluster
    ...
    def __init__(self, **config):
        ...
        self._namespace = config.get('namespace', 'rook-ceph')
        ...
        self._api_client = ac.get_api_client(self._api_client_name)
        self.ocs_pod_init()
    ...
    def ocs_pod_init(self):
        ...
        This is just an initializer for ```Class Pod``` from pod module
    ...
    def _get_pod_labels(self, pod_name, namespace):
        ...
        Return: labels from pod metadata
